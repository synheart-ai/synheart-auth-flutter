...
class PlatformBridge {
  ...
  Future<void> correctClockSkew(double serverTimestamp) async {
    await _invoke('correctClockSkew', {'serverTimestamp': serverTimestamp});
  }
        ...
        if (result is Map) {
          ...
          if (method == 'signRequest') {
            meta = {
              ...
              'noncePrefix':
                  m['nonce'] is String ? _preview(m['nonce'] as String) : null,
              'signatureLen': (m['signature'] is String)
                  ? (m['signature'] as String).length
                  : null,
